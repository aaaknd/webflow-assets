/*  app.js  — 3-D Glass Star cursor for Webflow
    Requires Three.js r150+ (load it before this file)
    Author: Perplexity AI • MIT licence
*/
class GlassLogoApp {
  constructor(container = document.body) {
    this.container = container;
    this.mouseNDC = new THREE.Vector2();       // cursor in Normalised Device Co-ords
    this._setupRenderer();
    this._setupScene();
    this._setupListeners();
    this._resize();            // initial size
    this._animate();           // start RAF loop
  }
  ...
  _setupScene() {
    ...
    // Environment capture for physically-based refraction
  ...
  _createStarMesh() {
    // Eight-point star derived from SVG, lightly bulged using vertex displacement
    const spikes       = 8;
    const outerRadius  = 1;
    ...
    /* --- subtle bulge toward the centre --- */
    const posAttr = geo.attributes.position;
    const v       = new THREE.Vector3();
    for (let i = 0; i < posAttr.count; i++) {
      v.fromBufferAttribute(posAttr, i);
      const dist = Math.sqrt(v.x * v.x + v.y * v.y);   // radial distance in XY
      v.z += (1 - dist) * 0.25;                        // raise Z proportionally
  ...
  _animate() {
    ...
    // follow cursor (mouseNDC maps to view space ~1 unit from camera)
    this.star.position.set(this.mouseNDC.x, this.mouseNDC.y, 0);

    // update environment for refraction
    this.star.visible = false;          // hide object while capturing env
...
/* ------ make globally accessible so you can call new GlassLogoApp(container) ------ */
